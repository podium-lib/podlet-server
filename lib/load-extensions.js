  ...
  async load() {
    ...
    for (const extensionPackage of extensionPackages) {
      if (Array.isArray(extensionPackage.package?.podium?.extension) && extensionPackage.package.podium.extension.includes("podlet-server")) {
        ...
        try {
          this.configSchemas.push((await import(configSchemaFile)).default);
        } catch (err) {
          // ignore
        }
        ...
        try {
          this.fallbackFiles.push((await import(fallbackFile)).default);
        } catch (err) {
          // ignore
        }
        ...
        try {
          this.buildPlugins.push((await import(buildPluginFile)).default);
        }
        catch (err) {
          // ignore
        }
        ...
        try {
          this.documentTemplates.push((await import(documentTemplateFile)).default);
        } catch (err) {
          // ignore
        }
        ...
        try {
          this.serverPlugins.push((await import(serverPluginFile)).default);
        } catch (err) {
          // ignore
        }
      }
