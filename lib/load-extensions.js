    ...
    const extensionPackages = await Promise.all(
      dependencies.map(async (dependency) => {
        ...
        return {
          ...
          package: JSON.parse(await fs.readFile(path.join(resolvedDependencyDir, "package.json"), "utf8")),
        };
      })
    );

    for (const extensionPackage of extensionPackages) {
      if (Array.isArray(extensionPackage.package?.podium?.extension) && extensionPackage.package.podium.extension.includes("podlet-server")) {
        ...
        try {
          this.configSchemas.push(await fs.readFile(configSchemaFile, { encoding: "utf8" }));
        } catch (err) {
          // ignore
        }
        ...
        try {
          this.fallbackFiles.push(await fs.readFile(fallbackFile, { encoding: "utf8" }));
        } catch (err) {
          // ignore
        }
        ...
        try {
          this.buildPlugins.push(await fs.readFile(buildPluginFile, { encoding: "utf8" }));
        }
        catch (err) {
          // ignore
        }
        ...
        try {
          this.documentTemplates.push(await fs.readFile(documentTemplateFile, { encoding: "utf8" }));
        } catch (err) {
          // ignore
        }
        ...
        try {
          this.serverPlugins.push(await fs.readFile(serverPluginFile, { encoding: "utf8" }));
        } catch (err) {
          // ignore
        }
      }
