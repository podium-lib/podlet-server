...
export default class FileUtilities {
  ...
  async resolve(path) {
    ...
    if (parse(resolution.path).ext === '') resolution.path += '.js';

    const meta = parse(resolution.path);
    const tsPath = join(meta.dir, `${meta.name}.ts`);
    const withTsExtension = meta.ext === '.ts';

    const [statsJs, statsTs] = await Promise.all([
      stat(resolution.path).catch(() => {}),
      stat(tsPath).catch(() => {}),
    ]);
  ...
  async buildAndResolve(path) {
    ...
    try {
      ...
      throw new Error(
        `Unable to convert TS file to JS for path ${resolution.path}`,
      );
    }
  ...
  async import(path) {
    ...
    throw new Error(
      `Unable to import file at path ${resolution.path}. File does not exist.`,
    );
  }
}
