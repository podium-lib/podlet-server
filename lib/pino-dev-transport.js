  ...
  pretty({
    ...
    messageFormat(log, messageKey) {
      ...
      if (method) {
        ...
        // response
        message = `[${chalk.cyan(reqId)}] ${coloriseStatusCode(
          statusCode,
        )} (${coloriseResponseTime(responseTime)})`;
      } else {
        message = `${msg}`;
      }
      ...
      if (level === 60) {
        const err = log.err
          ? `${chalk.bold.red(JSON.stringify(log.err, null, 2))}\n`
          : '';
      ...
      if (level === 50) {
        const err = log.err
          ? `${chalk.red(JSON.stringify(log.err, null, 2))}\n`
          : '';
