// @ts-ignore
import * as cliApi from '@lingui/cli/api'; // eslint-disable-line
...
export async function linguiCompile({ linguiConfig, config }) {
  try {
    ...
    if (locales.length === 0 || !configCatalogs || configCatalogs.length === 0)
      return;
    ...
    for (const locale of linguiConfig.locales) {
      for (const catalog of catalogs) {
        ...
        const compiledPath = await catalog.writeCompiled(
          locale,
          compiledCatalog,
          linguiConfig.compileNamespace,
        );
        ...
        logger.debug(
          chalk.magenta(
            `🌍 catalog rebuilt: ${locale} written to ${compiledPath}`,
          ),
        );
      }
    }
