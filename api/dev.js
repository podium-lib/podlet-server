...
export class DevServer {
  ...
  async #setup() {
    ...
    await linguiExtract({
      linguiConfig: this.linguiConfig,
      cwd: this.cwd,
      hideStats: true,
    });
    ...
    if (!contentFile.exists) {
      app.get('/', (request, reply) => {
        reply.redirect(
          join(this.config.get('app.base'), this.config.get('podlet.manifest')),
        );
      });
    }
    ...
    // if content file is defined, and content url doesn't resolve to /, redirect to content route
      ...
      app.get('/', (request, reply) => {
        reply.redirect(
          join(this.config.get('app.base'), this.config.get('podlet.content')),
        );
      });
    }
  ...
  async start() {
    const linguiConfig = await getLinguiConfig({
      config: this.config,
      cwd: this.cwd,
    });
    if (linguiConfig) {
