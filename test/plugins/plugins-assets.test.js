...
test('static assets not served for absolute base', async (t) => {
  ...
  await app.register(plugin, {
    cwd: tmp,
    base: 'https://assets.test.com/package',
  });
  ...
  t.equal(
    result.status,
    404,
    'test file should not be served at default location',
  );
  await app.close();
});
...
test('static assets served for relative base', async (t) => {
  ...
  t.equal(
    result.status,
    200,
    'test file should be served at configured location',
  );
  await app.close();
});
...
test('plugin mounted under a prefix: configured base', async (t) => {
  ...
  t.equal(
    result.status,
    200,
    'test file should be served under prefix and configured base',
  );
  await app.close();
});
