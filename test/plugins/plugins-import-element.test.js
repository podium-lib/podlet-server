...
beforeEach(async () => {
  ...
  await writeFile(
    join(tmp, 'element.js'),
    'export default class Element {static get replaced() {return false;}}',
  );
});
...
test('non development mode does not re-import element into registry with each call to importElement', async (t) => {
  ...
  // @ts-ignore
  t.equal(
    customElements.__definitions.get('test-app-element').ctor.replaced,
    false,
  );
});
...
test('throws if filepath given does not exist', async (t) => {
  ...
  try {
    ...
    t.match(
      err.message,
      `Cannot find module './does-not-exist.js'`,
      'should match expected error message',
    );
  }
});
...
test('successful server side rendering when dependent components used', async (t) => {
  ...
  // @ts-ignore
  t.equal(
    customElements.__definitions.get('test-app-element-with-dep').ctor.replaced,
    true,
  );
  // @ts-ignore
  t.equal(
    customElements.__definitions.get('dependent-component').ctor.replaced,
    true,
  );
});
