...
test('script function returns a script tag when development false', async (t) => {
  ...
  t.equal(
    app.script('/some/url'),
    `<script type="module" src="/some/url"></script>`,
...
test('script function returns a lazy script tag when lazy true', async (t) => {
  ...
  await app.register(plugin);
  t.equal(
    app.script('/some/url', { lazy: true }),
    `<script type="module">addEventListener('load',()=>import('/some/url'))</script>`,
    'should return a script tag',
  );
});
