...
test("root path handled via mappings plugin option", async (t) => {
  ...
  await app.register(plugin, { cwd: tmp, defaults, mappings: { "/": "route.json" } });
...
test("when validation schema is available and values don't validate, validation throws.", async (t) => {
  const { app } = t.context;
  await app.register(plugin, { cwd: tmp, defaults: { querystring: { count: { type: "integer" } } } });
  app.get("/route", () => {});
  ...
  t.equal(res.error, "querystring/count must be integer", "server should respond with a querystring error");
  await app.close();
  await rm(tmp, { recursive: true, force: true });
  t.end();
});
